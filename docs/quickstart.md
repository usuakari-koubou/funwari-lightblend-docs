...
> 消灯ライトマップ・Light Volume・リフレクションプローブの割り当ては、**ベイクの中で自動で行われます**。以前は「仕上げ」工程で手で押す必要がありましたが、押し忘れると枚数が食い違うためやめました。ライトマッパーを切り替えたときも、焼き直すだけで揃います。
...
旧バージョンでセットアップ済みのシーンを開くと、メインウィンドウ上部に「既存シーンの成立済み ○ 工程を完了として取り込めます」と表示されます。

1. シーンを保存します。
2. メインウィンドウを開きます。
3. 取り込みバーの「取り込む」を押します。
4. ガイドかダッシュボードで、再実行が必要な工程だけ進めます。

取り込みは既存の成果物を検査し、成立している工程だけを完了として記録します。
...
パッチ固有の注意事項は [パッチノート v1.0.0](patchnotes/v1_0_0) をご覧ください。
