...
> 消灯ライトマップ・Light Volume・リフレクションプローブの割り当ては、**ベイクの中で自動で行われます**。ライトマッパーを切り替えたときも、ベイクし直すだけで揃います。
...
- **シーンの外に書き出されたものは戻りません**（ベイク済みのライトマップなど）
...
ライトマッパーを切り替えると、ダッシュボードでは「ベイク」だけが要再実行になります。ベイクし直せば完了に戻ります。
