...
実行時に切り替えるスライダーは自動では置かれません。工程6「仕上げ」の「ボタン配置（任意）」の「昼夜のスライダー」で「昼夜のスライダーを置く」を押して置きます（昼夜を使うシーンでだけ出ます）。自分の Udon から動かしたい場合は `LightmapChanger.SetDaylight(float)`（0=夜 1=昼）を呼んでください。詳しくは [Settings.md の「自分の Udon から動かす」](settings.md#自分の-udon-から動かす)を参照してください。
